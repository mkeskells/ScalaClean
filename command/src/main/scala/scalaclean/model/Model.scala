...
sealed trait ClassLike extends ModelElement {
  ...
  /**
    ...
    */
  def extendsClassLikeCompiled(onlyDirect: Boolean = false, filter: ExtendsClassLike = All): Iterator[ClassLike] = {
    extendsClassLike0(onlyDirect, filter).flatMap(_.toElement)
  }
